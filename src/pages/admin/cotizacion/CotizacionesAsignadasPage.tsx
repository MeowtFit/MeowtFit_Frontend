...
function obtenerSesionUsuario() {
  ...
  const idComercianteTexto =
    localStorage.getItem("meowtfit_idComerciante") ||
    sessionStorage.getItem("meowtfit_idComerciante") ||
    localStorage.getItem("meowtfit_idUsuario") ||
    sessionStorage.getItem("meowtfit_idUsuario");

  const idComerciante = idComercianteTexto
    ? Number(idComercianteTexto)
    : null;
...
export default function CotizacionesAsignadasPage() {
  ...
  useEffect(() => {
    const sesionActual = obtenerSesionUsuario();

    if (!sesionActual.estaLogeado) {
      navigate("/login", {
        replace: true,
        state: {
          from: "/admin/cotizaciones/asignadas",
        },
      });
    ...
    if (!sesionActual.idComerciante) {
      setCargando(false);
      setError(
        "No se encontró el id del comerciante en la sesión. Asegúrate de guardar meowtfit_idComerciante o meowtfit_idUsuario al iniciar sesión."
      );
      return;
    }

    void cargarCotizaciones(0, "", sesionActual.idComerciante);
    void cargarResumen(sesionActual.idComerciante);
  }, [navigate]);

  async function cargarResumen(idComerciante: number) {
    try {
      ...
      // Si falla el resumen, no bloqueamos el listado principal.
    }
  }
  ...
  async function cargarCotizaciones(
    ...
    idComercianteParam = sesion.idComerciante
  ) {
    if (!idComercianteParam) {
      setError("No se pudo identificar al comerciante autenticado.");
      setCargando(false);
      return;
    }
    ...
    try {
      ...
      const data = await filtrarCotizaciones({
        estado: nuevoEstado,
        idComerciante: idComercianteParam,
        page: nuevaPagina,
      ...
      const message =
        err instanceof Error
          ? err.message
          : "No se pudieron cargar tus cotizaciones asignadas.";
  ...
  async function handleActualizar() {
    ...
    if (sesionActual.idComerciante) {
      await cargarResumen(sesionActual.idComerciante);
    }
  }
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#f7f5f2]">
        <p className="flex items-center gap-2 text-sm font-medium text-slate-500">
          ...
          Cargando cotizaciones asignadas...
        </p>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#f7f5f2] px-8 py-8">
      <section className="mx-auto max-w-7xl">
        ...
        <section className="mb-8 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
              Estas son las cotizaciones asociadas a tu usuario comerciante.
              Normalmente una cotización queda asociada cuando envías la primera
              contrapropuesta.
            </p>
          </div>
        ...
        <section className="mb-8 rounded-2xl border border-cyan-100 bg-cyan-50 px-5 py-4">
          <div className="flex items-start gap-3">
            <UserCheck className="mt-1 text-[#087f99]" size={22} />
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                ...
                · ID comerciante:{" "}
                <span className="font-bold text-slate-800">
                  {sesion.idComerciante ?? "No encontrado"}
                </span>
              </p>
            </div>
          </div>
        ...
        {cotizaciones.length === 0 ? (
          <section className="rounded-2xl bg-white p-16 text-center shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Cuando respondas una cotización con una contrapropuesta, debería
              quedar asociada a tu usuario comerciante.
            </p>
