...
export default function CotizacionAdminDetailPage() {
  const { id } = useParams();
  ...
  const puedeEnviarContrapropuesta =
    Boolean(cotizacion) &&
    estadoActual !== "CERRADA" &&
    estadoActual !== "RECHAZADA" &&
    !limiteAlcanzado;

  const puedeAceptarORechazar =
    Boolean(cotizacion) &&
    estadoActual !== "CERRADA" &&
    estadoActual !== "RECHAZADA";
  ...
  async function handleAceptarCotizacion() {
    ...
    try {
      ...
      setMensajeAccion("Cotización aceptada/cerrada correctamente.");
  ...
  async function handleEnviarContrapropuesta(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSustentoNuevo("");
      setMensajeAccion(
        "Contrapropuesta enviada correctamente. Si es la primera respuesta, esta cotización queda asociada al comerciante actual."
      );
  ...
  if (error && !cotizacion) {
    return (
      <main className="grid min-h-screen place-items-center bg-[#f7f5f2] px-6">
        ...
        <section className="grid gap-8 lg:grid-cols-[1fr_420px]">
          <div className="space-y-6">
                ...
                <div className="mt-6 rounded-2xl border border-cyan-100 bg-cyan-50 px-6 py-5">
                  ...
                  {diferenciaMonto !== null && (
                    <p className="mt-2 text-sm font-semibold text-slate-600">
                      Diferencia frente al total solicitado:{" "}
                      <span
                        className={
                          diferenciaMonto > 0
                            ? "text-red-500"
            ...
            <article className="rounded-2xl bg-white p-8 shadow-sm">
              ...
              {obtenerContrapropuestas(cotizacion).length === 0 ? (
                <p className="mt-5 rounded-xl bg-slate-50 px-5 py-4 text-sm text-slate-500">
                  Aún no hay contrapropuestas registradas.
                </p>
              ) : (
            ...
            {estadoActual === "CERRADA" ? (
              <div className="mt-6 rounded-xl bg-emerald-50 px-5 py-4">
                ...
                <p className="mt-2 text-sm text-emerald-600">
                  Esta cotización ya fue finalizada.
                </p>
              </div>
            ) : estadoActual === "RECHAZADA" ? (
              ...
              <>
                {puedeAceptarORechazar && (
                  <div className="mt-6 grid gap-3">
                    <Button
                      type="button"
                      onClick={() => void handleAceptarCotizacion()}
                      disabled={aceptando}
                      className="h-12 w-full rounded-xl bg-emerald-600 font-bold text-white hover:bg-emerald-700 disabled:opacity-60"
                    >
                      {aceptando && (
                        <Loader2 size={17} className="mr-2 animate-spin" />
                      )}
                      Aceptar / cerrar cotización
                    </Button>
                    ...
                    <Button
                      ...
                      disabled={rechazando}
                      className="h-12 w-full rounded-xl bg-red-500 font-bold text-white hover:bg-red-600 disabled:opacity-60"
                    >
                      {rechazando && (
                        <Loader2 size={17} className="mr-2 animate-spin" />
                      )}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          rows={4}
                          placeholder="Ej. Podemos atender el pedido a este precio..."
